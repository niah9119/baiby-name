#!/usr/bin/env bash
# watch-reasoning.sh — live view of what the agent is THINKING, not just doing.
#
# watch.sh streams tool calls only. This interleaves the model's reasoning text with a
# one-line summary of each tool call, so you can follow why it did something rather than
# just what it ran.
#
...
#
# Usage: ./watch-reasoning.sh [ISSUE_NUMBER]
