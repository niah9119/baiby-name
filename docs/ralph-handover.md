...
> Moved here from `tasks/todo.md`. The global CLAUDE.md instructs every agent to write its
> per-task plan to `tasks/todo.md`, so an agent overwrote this file during #17 — 160 lines of
> handover replaced by a 47-line implementation plan, which would have been destroyed on merge.
> `tasks/todo.md` belongs to whichever agent is running; durable notes live here.
